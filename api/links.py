...
from models.one_use_link import OneUseLinkCreate, OneUseLink, OneUseLinkDB
...
@router.post('/create_oneuse_link', response_model=OneUseLinkDB, status_code=status.HTTP_201_CREATED)
async def create_one_use_link(link: OneUseLinkCreate, user: User = Depends(auth_user)):

    if not user.id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail='User is incorrect')

    found = db_oneuse_link.search_by_id_and_redirect_url(user.id, link.redirect_url)

    if found:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                            detail='Link already exists in user account')
    
    new_oneuse_link = OneUseLink(
        user_id=user.id,
        redirect_url=link.redirect_url,
        small_url=create_random_url(6),
        create_at=str(datetime.utcnow())
    )

    new_oneuse_link_id = db_oneuse_link.insert_one_link(new_oneuse_link)

    created_link = db_oneuse_link.find_by_id(new_oneuse_link_id)

    if not created_link:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail='Cannot create in database')

    return OneUseLinkDB(**one_use_link_schema(created_link))